...
/*Edited code*/
std::string outputer = "/home/mike/snort/prediction/output_predict.txt";
...
std::ofstream outputFile("/home/mike/snort/prediction/output_predict.txt", std::ofstream::app);
/*End of edited code*/
...
/*Added new function*/
int model_predict(std::string ai_input) {
       
        std::string new_prediction = command + "\"" + ai_input + "\"";
        int returnValue = system(new_prediction.c_str());
        int exitCode = 0;
        
        if (WIFEXITED(returnValue))
        {
            stringList.push_back(outputer);
    ...
    const char* netbios_domain)
{
    assert(p);
    /*Edited code*/
    char total_flow_latency[TIMEBUF_SIZE];
    struct timeval diff;

    diff.tv_sec = p->pkth->ts.tv_sec - flow.flowstats.start_time.tv_sec;
    diff.tv_usec = p->pkth->ts.tv_usec - flow.flowstats.start_time.tv_usec;
    ...
    ts_print(&diff, total_flow_latency, true);

    long milliseconds = (diff.tv_sec * 1000) + (diff.tv_usec / 1000);

    snprintf(total_flow_latency, TIMEBUF_SIZE, "%ld", milliseconds);
    /*End of edited code*/
    ...
    js.put("client_bytes", flow.flowstats.client_bytes);
    js.put("server_bytes", flow.flowstats.server_bytes);
    js.put("total_flow_latency", total_flow_latency);
    ...
    if (!hsession)
    {
        ...
        /*added new variable ai_input*/
        ai_input = std::to_string(flow.flowstats.server_bytes) + ";,;" +
            (user_agent ? user_agent : "0") + ";,;" +
            std::to_string(flow.flowstats.client_pkts) + ";,;" +
            (payload_str ? payload_str : "0") + ";,;" +
            std::to_string(flow.flowstats.server_pkts) + ";,;" +
            (referrer ? referrer : "0") + ";,;" +
            (username ? username : "0") + ";,;" +
            (dns_host ? dns_host : "0") + ";,;" +
            (netbios_domain ? netbios_domain : "0") + ";,;" +
            get_proto_str(flow.ip_proto) + ";,;" +
            std::to_string(flow.flowstats.client_bytes) + ";,;" +
            (api.get_client_info(httpx_stream_index) ? api.get_client_info(httpx_stream_index) : "0") + ";,;" +
            (host ? host : "0") + ";,;" +
            (referred_str ? referred_str : "0") + ";,;" +
            (vendor ? vendor : "0") + ";,;" +
            (client_str ? client_str : "0") + ";,;" +
            (subtype ? subtype->version : "0") + ";,;" +
            std::to_string(id) + ";,;" +
            (misc_str ? misc_str : "0") + ";,;" +
            (service_str ? service_str : "0") + ";,;" +
            std::to_string(flow.server_port) + ";,;" +
            api.get_session_id() + ";,;" +
            (subtype ? subtype->service : "0")+ ";,;" +
            std::to_string(packet_num) + ";,;" +
            (version ? version : "0") + ";,;" +
            (tls_host ? tls_host : "0") + ";,;0;,;" +
            total_flow_latency + ";,;" +
            (netbios_name ? netbios_name : "0") + ";,;0;,;" +
            (response_code ? response_code : "0") + ";,;" +
            std::to_string(flow.client_port) + ";,;" +
            (url ? url : "0") + ";,;" +srv_ip_str;
    ...
    if (config.anomaly_detection) {
        ...
        if (outputFile.is_open()) {
            outputFile << ai_input << std::endl;
            flows_n++;
            if (flows_n % batchSize == 0) {
                if (config.anomaly_detection)
                    model_predict(outputFilePath);
                outputFile.close();
                std::string newFilePath = outputFilePath;
                newFilePath.insert(newFilePath.find_last_of('.'), "_" + std::to_string(fileCounter));
                outputFile.open(newFilePath, std::ofstream::app);
                outputer = newFilePath;
                if (!outputFile.is_open()) {
                    std::cout << "Nepodařilo se otevřít soubor." << std::endl;
